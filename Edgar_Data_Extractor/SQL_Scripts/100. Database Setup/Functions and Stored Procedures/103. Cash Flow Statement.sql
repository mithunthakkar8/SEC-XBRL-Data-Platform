/*
 * Function: create_cash_flow_statement_view
 * Description: Creates a dynamic view of cash flow statements for a given company
 * Parameters:
 *   p_ticker_symbol - The company's stock ticker symbol (e.g., 'FCX')
 *   p_exchange_code - The stock exchange code (e.g., 'NYQ')
 *   p_debug - Boolean flag to enable debug output (default FALSE)
 */
CREATE OR REPLACE FUNCTION company.create_cash_flow_statement_view(
    p_ticker_symbol VARCHAR,
    p_exchange_code VARCHAR,
    p_debug BOOLEAN DEFAULT FALSE
) RETURNS VOID AS $$
DECLARE
    dynamic_sql TEXT;         -- Stores the dynamically generated SQL
    column_list TEXT;         -- Stores the list of columns for pivot operation
    column_count INTEGER;     -- Count of distinct cash flow concepts
    view_name TEXT := format('%s_%s_cash_flow_statement', 
                           lower(p_ticker_symbol), 
                           lower(p_exchange_code));
BEGIN
    /*
     * STEP 1: Build the column list for the pivot operation
     * This identifies all cash flow concepts we need to include in the view
     */
    WITH cash_flow_concepts AS (
        -- Get distinct normalized concept names for cash flow items
        SELECT DISTINCT cnm.normalized_concept_name
        FROM company.CF_concept_name_mapping cnm
        JOIN xbrl.concept cpt ON cnm.original_concept_name = cpt.concept_name
    ...
    -- Convert concepts into CASE statements for pivot operation
    SELECT string_agg(
        format(
            'MAX(CASE WHEN normalized_concept_name = %L THEN normalized_numeric_value END) AS %I',
            normalized_concept_name,
            -- Convert camelCase to snake_case and limit to 63 chars
            lower(substring(regexp_replace(normalized_concept_name, '([a-z])([A-Z])', '\1_\2', 'g'), 1, 63))
        ),
        ', '
    ) INTO column_list
    FROM cash_flow_concepts;
    
    /*
     * STEP 2: Count distinct concept names for reporting/debug purposes
     */
    SELECT COUNT(*) INTO column_count 
    FROM (
        SELECT DISTINCT cnm.normalized_concept_name 
        FROM company.CF_concept_name_mapping cnm
        JOIN xbrl.concept cpt ON cpt.concept_name = cnm.original_concept_name
        WHERE cnm.ticker_symbol = p_ticker_symbol
          AND cnm.exchange_code = p_exchange_code
    ) AS t;
    ...
    IF p_debug THEN
        ...
        RAISE NOTICE 'Ticker: %, Exchange: %, Columns: %', 
                    p_ticker_symbol, p_exchange_code, column_count;
        RAISE NOTICE 'Column list: %', column_list;
    END IF;
    
    /*
     * STEP 3: Build and execute the dynamic SQL to create the view
     * The view combines reported data with derived quarterly data
     */
    dynamic_sql := format($sql$
        -- Drop existing view if it exists
        DROP VIEW IF EXISTS company.%I;
        
        -- Create new view with combined cash flow data
        CREATE VIEW company.%I AS
        /*
         * Subquery: latest_filing
         * Identifies the most recent filing for each concept/period combination
         */
        WITH latest_filing AS (
            ...
            JOIN xbrl.reported_fact rf ON c.context_id = rf.context_id 
                                      AND NOT rf.has_segment
        ...
        /*
         * Subquery: cash_flow_data
         * Retrieves and normalizes the actual cash flow values
         */
        cash_flow_data AS (
            SELECT 
                ...
                cp.period_start,
                cp.period_end,
                -- Normalize values (credit balances are negative)
                COALESCE(
                    CASE WHEN ca.balance_type = 'credit' THEN -rf.numeric_value 
                         ELSE rf.numeric_value 
                    END,
                    0
        ...
        -- Materialize the data and ensure all concepts have all period types
		materialized_data AS (
		    WITH all_concepts AS (
		        SELECT DISTINCT normalized_concept_name 
		        FROM cash_flow_data
		    ),
		    all_periods AS (
		        SELECT DISTINCT 
		            ...
		            report_year
		        FROM cash_flow_data
		        WHERE period_type IN ('Q1', 'Q2', 'Q3', 'Q4', 'Q2YTD', 'Q3YTD', 'Annual')
		    ),
		    cross_join AS (
		        SELECT 
		    ...
		    SELECT 
		        ...
		        cj.period_start,
		        cj.period_end,
		        COALESCE(cd.normalized_numeric_value, 0) AS normalized_numeric_value,
		        cj.period_type,
		        cj.report_year
		    FROM cross_join cj
        ...
        -- Filter to only standard reporting periods
        reported_periods AS (
        ...
        /*
         * Derived quarterly data sections
         * Calculate values for quarters that aren't directly reported
         */
        ...
        derived_q2 AS (
            SELECT 
                ...
                COALESCE(q2.normalized_numeric_value, 0) - COALESCE(q1.normalized_numeric_value, 0) AS normalized_numeric_value,
                'Q2' AS period_type,
                q1.report_year
        ...
        derived_q3 AS (
            SELECT 
                ...
                COALESCE(q3.normalized_numeric_value, 0) - COALESCE(q2.normalized_numeric_value, 0) AS normalized_numeric_value,
                'Q3' AS period_type,
                q2.report_year
        ...
        derived_q4 AS (
            SELECT 
                ...
                COALESCE(annual.normalized_numeric_value, 0) - COALESCE(q3.normalized_numeric_value, 0) AS normalized_numeric_value,
                'Q4' AS period_type,
                q3.report_year
        ...
        -- Final pivot operation to create columns for each cash flow concept
        SELECT 
            period_start,
            period_end,
            %s  -- This gets replaced with the dynamically generated column_list
        FROM combined_data
    ...
    -- Show full SQL if debug enabled
    IF p_debug THEN
    ...
    RAISE NOTICE 'View company.% created with % columns', view_name, column_count;
END;
$$ LANGUAGE plpgsql;
